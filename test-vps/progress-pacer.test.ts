...
describe("Telegram progress pacing", () => {
  it("uses a relaxed ten-second topic cadence by default", () => {
    const pacer = new TelegramProgressPacer();
    const start = 1_800_000_000_000;

    expect(pacer.tryAcquire(-1001, 10, start)).toBe(true);
    expect(pacer.tryAcquire(-1001, 10, start + 9_999)).toBe(false);
    expect(pacer.tryAcquire(-1001, 10, start + 10_000)).toBe(true);
  });
